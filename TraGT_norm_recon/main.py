...
def main(data_name,options):
    ...
    train_data['features'] = [min_max_norm(feature) for feature in train_data['features']]
    test_data['features'] = [min_max_norm(feature) for feature in test_data['features']]

    # Convert adjacency lists to adjacency matrices and normalize them
    adj_matrices_train = [min_max_norm(adj_list_to_adj_matrix(adj_list)) for adj_list in train_data['adj_lists']]
    adj_matrices_test = [min_max_norm(adj_list_to_adj_matrix(adj_list)) for adj_list in test_data['adj_lists']]

    # Stack sequences and normalize them
    data_sequence_train = min_max_norm(torch.stack(train_data['sequence']))
    data_sequence_test = min_max_norm(torch.stack(test_data['sequence']))

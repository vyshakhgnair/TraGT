...
class Graph_Transformer(nn.Module):
    def __init__(self, in_channels, hidden_channels_1,hidden_channels_2, out_channels, heads, dropout_rate=0.5):
        super(Graph_Transformer, self).__init__()
        self.device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.conv1 = GCNConvLayer(in_channels, hidden_channels_1, heads)
        self.conv2 = GCNConvLayer(hidden_channels_1 * heads, out_channels, heads)
        self.conv3 = GCNConvLayer(hidden_channels_2*heads, out_channels, heads)
